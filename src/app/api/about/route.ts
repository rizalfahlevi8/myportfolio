...
export async function POST(req: NextRequest) {
  try {
    ...
    const skillId = JSON.parse(
      (formData.get("skillId") as string) || "[]",
    ) as string[];
    const sosmedId = JSON.parse(
      (formData.get("sosmedId") as string) || "[]",
    ) as string[];
    const projectId = JSON.parse(
      (formData.get("projectId") as string) || "[]",
    ) as string[];
    const workExperienceId = JSON.parse(
      (formData.get("workExperienceId") as string) || "[]",
    ) as string[];
